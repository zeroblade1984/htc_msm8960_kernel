	...
	if (unlock == 2 && ts->first_pressed && ts->pre_finger_data[0][0] < 2) {
		...
		if(ts->psensor_detection) {
			if(ts->psensor_resume_enable == 1) {
				pr_debug(KERN_INFO "[TP] %s: Disable P-sensor by Touch\n", __func__);
	...
	unsigned long status, void *unused)
{
	...
	pr_debug(KERN_INFO "[TP] psensor status %d -> %lu\n",
...
static int synaptics_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if (ts->packrat_number < SYNAPTICS_FW_NOCAL_PACKRAT)
		pr_debug(KERN_INFO "[TP][PWR][STATE] get power key state = %d\n", getPowerKeyState());
#endif
	if (ts->disable_CBC) {
		if (ts->package_id < 3400) {
			ret = i2c_syn_read(ts->client,
...
static int synaptics_ts_resume(struct i2c_client *client)
{
	...
	if (ts->psensor_detection) {
		if((ts->psensor_status & PSENSOR_STATUS) == 0) {
			...
			pr_debug(KERN_INFO "[TP] %s: Enable P-sensor by Touch\n", __func__);
